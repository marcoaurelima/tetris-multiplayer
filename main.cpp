...
#include <unistd.h> //renomea�ao de arquivos
#include <iostream>
#include <stdio.h>
#include <time.h>
#include <stdlib.h>
...
sf::RenderWindow janela(sf::VideoMode(1050, 591), "Tetris multiplayer");
//janela.setFramerateLimit(10);
bool JogoRodando=true; //vai garantir um loop entre todas as telas
int CodTela=1;//cada tela tem um codigo, e retorna um inteiro que � o codigo da proxima tela
...
int telaEmpate(){
     ...
     sf::SoundBuffer somBuff;
	somBuff.loadFromFile("Sons/somEmpate.ogg");
	sf::Sound som;
	som.setBuffer(somBuff);
	som.setVolume(90.f);
	som.play();

    sf::Texture textura;
   textura.loadFromFile("img/Empate.png");
   sf::Sprite sprite;
...
int TelaMesmosJogadores(){
     ...
     sf::SoundBuffer somBuff;
	somBuff.loadFromFile("Sons/somTransicao.ogg");
	sf::Sound som;
	som.setBuffer(somBuff);
	som.setVolume(90.f);
	som.play();
    ...
    sf::Event evento;
    	while (janela.pollEvent(evento)){
            ...
            if(evento.key.code==sf::Keyboard::Escape){
                file=std::fopen("Data/Players.txt", "w");//se for outros jogadores, ele subscreve o arquivo em branco
                std::fclose(file);
                CodTela=3;
...
int TelaRecorde(){
    ...
    sf::SoundBuffer somBuff;
	somBuff.loadFromFile("Sons/somRec.ogg");
	sf::Sound som;
	som.setBuffer(somBuff);
	som.setVolume(90.f);
	som.play();
    ...
    FILE *file;
    file=std::fopen("Data/Ranking_1_Lugar.txt", "r");
   ...
   sf::Font fonteArcade;;
   fonteArcade.loadFromFile("Fontes/ARCADECLASSIC.otf");
    ...
    sf::FloatRect textRect1 = textoNome.getLocalBounds();
    textoNome.setOrigin(textRect1.width/2,textRect1.height/2);
    textoNome.setPosition(sf::Vector2f(1050/2.0f,294.f));
    ...
    sf::FloatRect textRect2 = textoPontos.getLocalBounds();
    textoPontos.setOrigin(textRect2.width/2,textRect2.height/2);
    textoPontos.setPosition(sf::Vector2f(1050/2.0f,348.f));
...
int TelaMenuPrincipal(){
    ...
    sf::Music musica;
	musica.openFromFile("Sons/musica_menu.ogg");
    musica.play();
	musica.setLoop(true);
	musica.setVolume(90.f);
...
int TelaRanking(){
    ...
    sf::SoundBuffer somBuff;
	somBuff.loadFromFile("Sons/somRanking.ogg");
	sf::Sound som;
	som.setBuffer(somBuff);
	som.setVolume(50.f);
	som.play();
   ...
   char n1[10], n2[10], n3[10], n4[10];//variaveis provisoria, pois FSCANF nao atribui diretamente na posi�ao da matriz a string, tem que ter esse intermediario


   FILE *file1=std::fopen("Data/Ranking_1_Lugar.txt","r");
std::fscanf(file1,"%s%s%s%s\n",n2,n1,n3,n4);
   ...
   FILE *file2=std::fopen("Data/Ranking_2_Lugar.txt","r");
std::fscanf(file2,"%s%s%s%s\n",n2,n1,n3,n4);
   ...
   FILE *file3=std::fopen("Data/Ranking_3_Lugar.txt","r");
std::fscanf(file3,"%s%s%s%s\n",n2,n1,n3,n4);
   ...
   sf::Font fonte;
   fonte.loadFromFile("Fontes/ARCADECLASSIC.otf");
   sf::Text nome1;
   nome1.setFont(fonte);
...
int TelaVencedor(){
    ...
    sf::SoundBuffer somPontBuff;
	somPontBuff.loadFromFile("Sons/somVitoria.ogg");
	sf::Sound som;
	som.setBuffer(somPontBuff);
	som.setVolume(90.f);
	som.play();
   ...
   FILE *file1=std::fopen("Data/VencedorPartidaAtual.txt", "r");
  ...
  // nome1.setPosition(449.f,174.f);
    ...
    sf::FloatRect textRect1 = nome1.getLocalBounds();
    nome1.setOrigin(textRect1.width/2,textRect1.height/2);
    nome1.setPosition(sf::Vector2f(1050/2.0f,184.f));
   ...
   sf::FloatRect textRect2 = nome2.getLocalBounds();
    nome2.setOrigin(textRect2.width/2,textRect2.height/2);
    nome2.setPosition(sf::Vector2f(1050/2.0f,235.f));
...
int TelaJog2(){
   ...
   sprite.setPosition(0,0);
    ...
    FILE *file=std::fopen("Data/Players.txt", "a");

    sf::Event evento;
    sf::Font fonte;
    fonte.loadFromFile("Fontes/MotionControl-Bold.otf");
    sf::Text nomJog1;
...
int TelaJog1(){
    ...
    sf::SoundBuffer somBuff;
	somBuff.loadFromFile("Sons/somNomes.ogg");
	sf::Sound som;
	som.setBuffer(somBuff);
	som.setVolume(70.f);
	som.play();
...
int TelaJogo(){
   ...
   char jogRecorde[10], pontosRecorde[10];
    ...
    FILE *file=std::fopen("Data/Players.txt", "r");
    std::fscanf(file,"%s%s\n", n1, n2);
    std::fclose(file);

    FILE *file2=std::fopen("Data/Ranking_1_Lugar.txt", "r");
   ...
   sf::FloatRect textRectNome1 = nomJog1.getLocalBounds();
    nomJog1.setOrigin(textRectNome1.width/2,textRectNome1.height/2);
    nomJog1.setPosition(sf::Vector2f(290.f,78.f));
   ...
   sf::FloatRect textRectNome2 = nomJog2.getLocalBounds();
    nomJog2.setOrigin(textRectNome2.width/2,textRectNome2.height/2);
    nomJog2.setPosition(sf::Vector2f(755.f,78.f));
	...
	int i, j, mov_y=0, mov_x=4,mov_x_2=4,mov_y_2=0; //mov_y e mov_x � usado para incrementar a pe�a na matriz principal, no eix x e eixo y | MOV_X=4 para come�ar no meio
	int delay=3,delay_2=2; // serve para dar um delay de 3 segundos antes da pe�a descer
	...
	int limitedapeca_y=0,limitedapeca_y_2; //valor que vai controlar o limite da pe�a em rela�ao a borda inferior. ele � igual ao maior valor da pe�a no eixo y;
	int limitedapeca_x=0,limitedapeca_x_2; //valor que vai controlar o limite da pe�a em rela�ao a borda inferior. ele � igual ao maior valor da pe�a no eixo x;
	...
	for(i=0;i<10;i++){
    	for(j=0;j<20;j++){
        	matrizdevalxy[i][j].px=(i*20.8)+189.0; //i multiplica pois � a coluna ; 20.8 px � o recuo da pe�a no eixo x quando pressionado (largura)
	...
	sf::Music musica;
	musica.openFromFile("Sons/musica_no_jogo.ogg");
    musica.play();
	musica.setLoop(true);
	musica.setVolume(90.f);

	sf::SoundBuffer somPontBuff;
	somPontBuff.loadFromFile("Sons/Ponto.ogg");
	sf::Sound somPonto;
	somPonto.setBuffer(somPontBuff);
	somPonto.setVolume(100.f);
	...
	sf::Font fonteMotion;
	fonteMotion.loadFromFile("Fontes/MotionControl-Bold.otf");
    ...
    for(i=0;i<2;i++){
	    proximapeca_2[i]=rand()%9; //para nao gerar pe�as de codigo 3 e 4 pois tem redundancia com outras pe�as giradas
    ...
    FILE *fileRanking=std::fopen("Data/Ranking_1_Lugar.txt","r");//esta pegando a pontua�ao do 1 lugar como base para saber se tem recorde
...
*/
    ...
    sf::FloatRect textRectRec1 = textoArcade3.getLocalBounds();
    textoArcade3.setOrigin(textRectRec1.width/2,textRectRec1.height/2);
    textoArcade3.setPosition(sf::Vector2f(1050/2.0f,430.f));
    ...
    sf::FloatRect textRectRec2 = textoArcade4.getLocalBounds();
    textoArcade4.setOrigin(textRectRec2.width/2,textRectRec2.height/2);
    textoArcade4.setPosition(sf::Vector2f(1050/2.0f,455.f));
    ///==============================================
   ...
   temposegundos = (int) clockCronometro.getElapsedTime().asSeconds(); //esta sendo feito o casting de float para inteiro
    ...
    while(i>0){
    	...
    	for(i2=i-1;i2>0;i2--){ // esses 2 la�os tranferem todas as linhas acima da linha apagada pra baixo;
        	for(j2=0;j2<10;j2++){ //m
                if(matrizPlayerEstatico_2[j2][i2]==true&&i2>0){ //impede que na linha zero, ele pegue o lixo de memoria, pois n�o existe nada acima da linha 0;
    ...
    while(i>0){
    	...
    	if(clock_2.getElapsedTime().asSeconds()>0.3){
       	...
       	if(delay_2==0 && mov_y_2<=18-limitedapeca_y_2&&colisao_Y_2==false){ // limite de incrementa�ao ate em baixo vairoa com o tamanho da pe�a (limitepeca)
        	mov_y_2++;
       	}
            	...
            	if(delay_2==0){
                    	...
                    	for(i=0;i<10;i++){
                        	for(j=0;j<20;j++){
                            	if(matrizPlayer_2[i][j]){
                            	matrizdecodcoresparamatrizsecund_2[i][j] = aleatorioCor_2;// toda vez que acontecer a copia sera copiado tmb o codigo da cor da pe�a em questao
            	...
            	if(delay_2==0&&jogador2perdeu==false){//se jogador 2 perdeu, nao sera mais feito pe�as aleatorias
        ...
        colisao_Y_2=false; //reseta a condi�ao de colisao
    	...
    	if(clock.getElapsedTime().asSeconds()>0.3){
       	...
       	if(delay==0 && mov_y<=18-limitedapeca_y&&colisao_Y==false){ // limite de incrementa�ao ate em baixo vairoa com o tamanho da pe�a (limitepeca)
        	mov_y++;
            	...
            	//#################  PRODUZ OUTRA COR E OUTRO FORMATO #####################
            	if(delay==0&&jogador1perdeu==false){//se jogador 1 perdeu, nao sera mais feito pe�as aleatorias
        ...
        colisao_Y=false; //reseta a condi�ao de colisao
        ...
        for(i=0;i<10;i++){
            if(!jogador1perdeu){//condicional para ele so entrar uma vez, pois tem algoritmo de grava�ao em arquivo

                if(matrizPlayerEstatico[i][0]==true){
                    if(jogador1perdeu==false&&jogador2perdeu==false){
                    FILE *fileResultado=std::fopen("Data/ResultadoDoJogo.txt", "w");
                    std::fclose(fileResultado);
                    }

                    FILE *fileResultado1=std::fopen("Data/ResultadoDoJogo.txt", "a");
                    std::fprintf(fileResultado1, "%d %s %d %d ", pontos, n1, tempominutos, temposegundos);
                    std::fclose(fileResultado1);
                    jogador1perdeu=true;
                }
            }
            ...
            if(!jogador2perdeu){
                ...
                if(matrizPlayerEstatico_2[i][0]==true){
                    ...
                    FILE *fileResultado2=std::fopen("Data/ResultadoDoJogo.txt", "a");
                    std::fprintf(fileResultado2, "%d %s %d %d ", pontos_2, n2, tempominutos, temposegundos);
                    std::fclose(fileResultado2);
                    jogador2perdeu=true;
                }
            }
...
int quemGanhou(){
    ...
    bool empate=false;
    int somaTempo1, somaTempo2;

    char nomeJog1[10];
    int pontos_1, min_1, seg_1;
    char nomeJog2[10];
    int pontos_2, min_2, seg_2;

    char Nome1lugar[10];
    int pontos1lugar, min1lugar, seg1lugar;

    char Nome2lugar[10];
    int pontos2lugar, min2lugar, seg2lugar;

    char Nome3lugar[10];
    int pontos3lugar, min3lugar, seg3lugar;



    FILE *file=std::fopen("Data/ResultadoDoJogo.txt", "r");
    std::fscanf(file,"%d %s %d %d %d %s %d %d\n", &pontos_1, nomeJog1, &min_1, &seg_1, &pontos_2, nomeJog2, &min_2, &seg_2);
    fclose(file);

        file=std::fopen("Data/Ranking_1_Lugar.txt", "r");
    std::fscanf(file,"%d %s %d %d\n",&pontos1lugar, Nome1lugar, &min1lugar, &seg1lugar);
    fclose(file);

        file=std::fopen("Data/Ranking_2_Lugar.txt", "r");
    std::fscanf(file,"%d %s %d %d\n",&pontos2lugar, Nome2lugar, &min2lugar, &seg2lugar);
    fclose(file);

        file=std::fopen("Data/Ranking_3_Lugar.txt", "r");
    std::fscanf(file,"%d %s %d %d\n",&pontos3lugar, Nome3lugar, &min3lugar, &seg3lugar);
    fclose(file);



    std::printf("jogador 1: %d %s %d %d\n", pontos_1, nomeJog1, min_1, seg_1);
    std::printf("jogador 2: %d %s %d %d\n", pontos_2, nomeJog2, min_2, seg_2);
    std::printf("1 lugar: %d %s %d %d\n", pontos1lugar, Nome1lugar, min1lugar, seg1lugar);
    std::printf("2 lugar: %d %s %d %d\n", pontos2lugar, Nome2lugar, min2lugar, seg2lugar);
    std::printf("3 lugar: %d %s %d %d\n", pontos3lugar, Nome3lugar, min3lugar, seg3lugar);

    FILE *fileVencedor=std::fopen("Data/VencedorPartidaAtual.txt", "w");

    somaTempo1=(min_1*60)+seg_1;
    somaTempo2=(min_2*60)+seg_2;

    if(pontos_1!=pontos_2){
        if(pontos_1>pontos1lugar){
            std::rename("Data/Ranking_1_Lugar.txt", "Data/Ranking_1_Lugar(alt).txt");
            std::rename("Data/Ranking_2_Lugar.txt", "Data/Ranking_2_Lugar(alt).txt");
            std::rename("Data/Ranking_3_Lugar.txt", "Data/Ranking_3_Lugar(alt).txt");

            std::rename("Data/Ranking_1_Lugar(alt).txt", "Data/Ranking_2_Lugar.txt");
            std::rename("Data/Ranking_2_Lugar(alt).txt", "Data/Ranking_3_Lugar.txt");
            std::rename("Data/Ranking_3_Lugar(alt).txt", "Data/Ranking_1_Lugar.txt");

            file=std::fopen("Data/Ranking_1_Lugar.txt", "w");
            std::fprintf(file, "%d %s %d %d\n", pontos_1, nomeJog1, min_1, seg_1);
            std::fclose(file);

            std::fprintf(fileVencedor, "%s %d\n", nomeJog1, pontos_1);
            std::fclose(fileVencedor);

            CodTela=6;
            return 0;
        } else if(pontos_2>pontos1lugar){
            std::rename("Data/Ranking_1_Lugar.txt", "Data/Ranking_1_Lugar(alt).txt");
            std::rename("Data/Ranking_2_Lugar.txt", "Data/Ranking_2_Lugar(alt).txt");
            std::rename("Data/Ranking_3_Lugar.txt", "Data/Ranking_3_Lugar(alt).txt");
            ...
            CodTela=6;
            return 0;
        } else if(pontos_1>pontos2lugar&&pontos_1>pontos3lugar){//maior que 2 e 3 lugar
            std::rename("Data/Ranking_2_Lugar.txt", "Data/Ranking_2_Lugar(alt).txt");
            std::rename("Data/Ranking_3_Lugar.txt", "Data/Ranking_3_Lugar(alt).txt");


            std::rename("Data/Ranking_2_Lugar(alt).txt", "Data/Ranking_3_Lugar.txt");
            std::rename("Data/Ranking_3_Lugar(alt).txt", "Data/Ranking_2_Lugar.txt");

            file=std::fopen("Data/Ranking_2_Lugar.txt", "w");
            std::fprintf(file, "%d %s %d %d\n", pontos_1, nomeJog1, min_1, seg_1);
            std::fclose(file);

            std::fprintf(fileVencedor, "%s %d\n", nomeJog1, pontos_1);
            std::fclose(fileVencedor);
        } else if(pontos_2>pontos2lugar&&pontos_2>pontos3lugar){//maior que 2 e 3 lugar
            std::rename("Data/Ranking_2_Lugar.txt", "Data/Ranking_2_Lugar(alt).txt");
            std::rename("Data/Ranking_3_Lugar.txt", "Data/Ranking_3_Lugar(alt).txt");
            ...
            std::fprintf(fileVencedor, "%s %d\n", nomeJog2, pontos_2);
            std::fclose(fileVencedor);
        } else if(pontos_1<pontos2lugar&&pontos_1>pontos3lugar){//menor q 2 maior q 3 lugar
            file=std::fopen("Data/Ranking_3_Lugar.txt", "w");
            std::fprintf(file, "%d %s %d %d\n", pontos_1, nomeJog1, min_1, seg_1);
            std::fclose(file);

            std::fprintf(fileVencedor, "%s %d\n", nomeJog1, pontos_1);
            std::fclose(fileVencedor);
        } else if(pontos_2<pontos2lugar&&pontos_2>pontos3lugar){//menor q 2 maior q 3 lugar
            file=std::fopen("Data/Ranking_3_Lugar.txt", "w");
            std::fprintf(file, "%d %s %d %d\n", pontos_2, nomeJog2, min_2, seg_2);
            std::fclose(file);
            ...
            std::fprintf(fileVencedor, "%s %d\n", nomeJog2, pontos_2);
            std::fclose(fileVencedor);
        } else if(pontos_1>pontos_2&&pontos_1<pontos1lugar&&pontos_1<pontos2lugar&&pontos_1<pontos3lugar){
            std::fprintf(fileVencedor, "%s %d\n", nomeJog1, pontos_1);
            std::fclose(fileVencedor);
        } else if(pontos_2>pontos_1&&pontos_2<pontos1lugar&&pontos_2<pontos2lugar&&pontos_2<pontos3lugar){
            std::fprintf(fileVencedor, "%s %d\n", nomeJog2, pontos_2);
            std::fclose(fileVencedor);
        } else if(pontos_1>pontos_2&&pontos_1==pontos1lugar||pontos_1==pontos2lugar||pontos_1==pontos3lugar){
            std::fprintf(fileVencedor, "%s %d\n", nomeJog1, pontos_1);
        ...
        } else if(somaTempo2>somaTempo1){
            if(pontos_2>pontos1lugar){
                std::rename("Data/Ranking_1_Lugar.txt", "Data/Ranking_1_Lugar(alt).txt");
                std::rename("Data/Ranking_2_Lugar.txt", "Data/Ranking_2_Lugar(alt).txt");
                std::rename("Data/Ranking_3_Lugar.txt", "Data/Ranking_3_Lugar(alt).txt");
    ...
    if(empate){
        CodTela=9;//tela empate
        return 0;
    } else {
        CodTela=7;// tela vencedor
        return 0;
    }
...
int main(){
    //essa parte serve pra deixar os Data/Players em branco, pois a cada nova execu�ao, tem que come�ar sem jogadores no arquivo
    FILE *file;
    file=std::fopen("Data/Players.txt", "w");
    std::fclose(file);
    //**************
